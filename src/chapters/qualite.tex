...
Chaque risque de l'OWASP Top 10 (2025) est associé à une mesure concrète mise en place dans BrackX. Ces mesures relèvent de bonnes pratiques appliquées au niveau applicatif~: elles réduisent la surface d'exposition sans prétendre à une conformité certifiée, qui supposerait un audit dédié.
...
La seule donnée personnelle collectée à ce stade est l'adresse email, qui sert d'identifiant de connexion~: aucune autre information nominative n'est demandée, ce qui limite d'emblée l'exposition au regard du RGPD. Le volet conformité proprement dit reste toutefois à développer et constitue un chantier identifié avant la mise en production, et non une fonctionnalité déjà livrée. Deux mesures sont prévues~: la purge ou l'anonymisation des tournois laissés inactifs, par exemple l'anonymisation des équipes au bout d'un mois sans connexion du propriétaire, et la désactivation des comptes inactifs après une période de deux à six mois sans connexion. Ce point est assumé comme un risque à traiter, cohérent avec la trajectoire de mise en production.

\subsection{Accessibilité — RGAA 4.1}
Le référentiel retenu est le RGAA 4.1 (Référentiel Général d'Amélioration de l'Accessibilité), référence officielle française compatible avec les exigences légales applicables aux services numériques. Plusieurs mesures rapprochent le prototype de ce référentiel. Tous les éléments interactifs (boutons, liens, champs) portent un attribut \texttt{aria-label} ou \texttt{aria-labelledby} décrivant leur fonction, et les formulaires de configuration associent explicitement chaque label à son champ (\texttt{for}/\texttt{id}). Le contraste des textes vise le ratio minimum de 4.5:1 préconisé par le WCAG AA. La navigation au clavier est fonctionnelle sur l'ensemble de l'application, avec une tabulation logique et un focus visible, et les messages d'erreur de formulaire sont annoncés aux lecteurs d'écran via \texttt{aria-live}.

Ces éléments constituent un rapprochement du référentiel, non une conformité mesurée. La navigation au clavier a été éprouvée lors des tests structurels de la recette (compétence C2.3.1), mais aucun audit RGAA formel, ni déclaration de conformité, ni test avec lecteur d'écran n'ont encore été conduits. Un audit outillé (par exemple Lighthouse ou axe-core) reste à mener pour objectiver et chiffrer le niveau réellement atteint. Une limite est par ailleurs déjà identifiée~: la vue bracket en arbre (visualisation des phases d'élimination) est difficile à rendre pleinement accessible, ce pour quoi une alternative textuelle sous forme de tableau de résultats est proposée en complément du bracket visuel.
...
Le point le plus structurant est que le schéma de données porte déjà les liaisons entre matchs~: le vainqueur d'un match alimente automatiquement le match suivant. Aujourd'hui, un template génère ces liaisons selon des règles prédéfinies, mais rien n'empêche techniquement de les définir librement, à la main, par exemple par glisser-déposer. Une telle édition libre des tournois ne remettrait pas en cause le back-end, dont la logique de propagation resterait identique~: l'effort se concentrerait sur l'expérience de construction côté front. Les évolutions planifiées suivent d'ailleurs ce principe. Le partage public d'un tournoi ne nécessite que l'ajout d'un champ \texttt{isPublic} sur l'entité \texttt{Tournoi} et d'un guard autorisant la lecture non authentifiée, et les notifications temps réel s'appuieront sur le support natif des WebSockets par NestJS (\texttt{@WebSocketGateway}), sans impacter les modules existants.

L'évolutivité fonctionnelle ne doit pas être confondue avec la montée en charge. L'architecture actuelle regroupe le frontend, le back-end et la base de données dans une même image, ce qui convient au volume attendu mais n'est pas dimensionné pour une très forte charge simultanée~: le point de rupture serait le couple back-end et base de données. Une montée à grande échelle passerait par la segmentation des services et une orchestration (Kubernetes) capable de dimensionner chaque service indépendamment selon des seuils de charge. Ce chantier est volontairement écarté à ce stade, où il serait surdimensionné pour le nombre d'utilisateurs visé, et relève de la supervision étudiée au Bloc 4.
